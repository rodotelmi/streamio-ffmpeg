...
module FFMPEG
  class Movie
    ...
    def initialize(path)
      raise Errno::ENOENT, "the file '#{path}' does not exist" unless !path.match(/^(ftp|hls|http|https|rtsp|rtmp|rtmps|rtmpt|rtmpts|rtp|srtp|tls|udp|mms|mmsh|mmst)/).nil? || File.exists?(path)
      ...
      Open3.popen3(command) do |i,o,e,w|
        begin
          Timeout.timeout(@@timeout) do # 
            ...
            std_output = o.read unless o.nil?
            std_error = e.read unless e.nil?
          end
        rescue Timeout::Error
          # here you know that the process took longer than 30 seconds
